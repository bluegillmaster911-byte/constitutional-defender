import {
  ...
  MAX_TIER,
  CASE_REWARDS,
  OBJECTION_SUCCESS_REPUTATION,
  OBJECTION_FAILURE_REPUTATION,
} from "../constants/game";
import type { PlayerState, VerdictResult, Tier, EvidenceItem } from "../types/game";
...
export function unlockEvidence(
  ...
  if (currentEvidence.includes(newEvidenceId)) {
    return currentEvidence;
  }
  return [...currentEvidence, newEvidenceId];
}

export function calculateEvidenceScore(evidence: EvidenceItem[]): number {
  if (evidence.length === 0) return 0;

  const total = evidence.reduce((sum, item) => {
    return sum + (item.strength + item.credibility + item.relevance) / 3;
  }, 0);

  return total / evidence.length;
}

export function canUnlockHiddenEvidence(
  trustLevel: number,
  specificActionTaken: boolean
): boolean {
